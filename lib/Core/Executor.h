...
private:
  ...
  //ExecutionState* findThreadByThreadId(unsigned threadId);

  //PTree* createNewPtree(unsigned threadId, ExecutionState* state);

  //bool isAllThreadTerminate();
  ...
  std::vector<BitcodeListener*> bitcodeListeners; // listener list
...
public:
  ...
  void runVerification(llvm::Function *f,
                                     int argc,
                                     char **argv,
                                     char **envp);
