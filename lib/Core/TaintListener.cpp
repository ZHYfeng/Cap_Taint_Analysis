...
void TaintListener::executeInstruction(ExecutionState &state, KInstruction *ki) {
	Trace* trace = rdManager->getCurrentTrace();
	if ((*currentEvent)) {
...
void TaintListener::instructionExecuted(ExecutionState &state, KInstruction *ki) {
	Trace* trace = rdManager->getCurrentTrace();
	if ((*currentEvent)) {
		Instruction* inst = ki->inst;
		Thread* thread = state.currentThread;
...
ref<Expr> TaintListener::manualMakeTaint(ExecutionState& state,
