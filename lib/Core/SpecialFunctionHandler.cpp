...
#define addDNR(name, handler) { name, \
		...
		addDNR("_exit", handleExit), { "exit",
				&SpecialFunctionHandler::handleExit, true, false, true },
...
std::string SpecialFunctionHandler::readStringAtAddress(ExecutionState &state,
	...
	assert(
			executor.solver->mustBeTrue(state,
					EqExpr::create(address, op.first->getBaseExpr()), res)
...
void SpecialFunctionHandler::handleAssume(ExecutionState &state,
	...
	if (res) {
		executor.terminateStateOnError(state,
				"invalid klee_assume call (provably false)", "user.err");
	} else {
...
void SpecialFunctionHandler::handleSetForking(ExecutionState &state,
	...
	if (ConstantExpr *CE = dyn_cast<ConstantExpr>(value)) {
		...
		executor.terminateStateOnError(state,
				"klee_set_forking requires a constant arg", "user.err");
	}
}
...
void SpecialFunctionHandler::handleCheckMemoryAccess(ExecutionState &state,
	...
	if (!isa<ConstantExpr>(address) || !isa<ConstantExpr>(size)) {
		executor.terminateStateOnError(state,
				"check_memory_access requires constant args", "user.err");
	} else {
		ObjectPair op;
